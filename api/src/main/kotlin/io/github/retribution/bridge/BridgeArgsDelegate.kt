...
internal typealias MethodArgs = List<Any?>
internal typealias NativeObject = Any?
...
class ArgsDelegate(source: List<Any?>) {
    // Snapshot in case the list is mutated elsewhere.
    private val raw: List<Any?> = source.toList()
    private var cursor: Int = 0

    private fun sequential(): Accessor = Accessor(raw) { captureIndex() }

    fun string(default: String? = null): ReadOnlyProperty<Any?, String> = sequential().string(default)
    fun stringOrNull(): ReadOnlyProperty<Any?, String?> = sequential().stringOrNull()

    fun int(default: Int? = null): ReadOnlyProperty<Any?, Int> = sequential().int(default)
    fun intOrNull(): ReadOnlyProperty<Any?, Int?> = sequential().intOrNull()

    fun double(default: Double? = null): ReadOnlyProperty<Any?, Double> = sequential().double(default)
    fun doubleOrNull(): ReadOnlyProperty<Any?, Double?> = sequential().doubleOrNull()

    fun boolean(default: Boolean? = null): ReadOnlyProperty<Any?, Boolean> = sequential().boolean(default)
    fun booleanOrNull(): ReadOnlyProperty<Any?, Boolean?> = sequential().booleanOrNull()

    fun arrayList(default: ArrayList<String>? = null): ReadOnlyProperty<Any?, ArrayList<String>> =
        sequential().arrayList(default)

    fun arrayListOrNull(): ReadOnlyProperty<Any?, ArrayList<String>?> =
        sequential().arrayListOrNull()

    @JvmName("arrayListTyped")
    fun <T> arrayList(default: ArrayList<T>? = null): ReadOnlyProperty<Any?, ArrayList<T>> =
        sequential().arrayList(default)
    ...
    fun hashMap(default: HashMap<String, Any?>? = null): ReadOnlyProperty<Any?, HashMap<String, Any?>> =
        sequential().hashMap(default)

    fun hashMapOrNull(): ReadOnlyProperty<Any?, HashMap<String, Any?>?> =
        sequential().hashMapOrNull()

    @JvmName("hashMapTyped")
    fun <K, V> hashMap(default: HashMap<K, V>? = null): ReadOnlyProperty<Any?, HashMap<K, V>> =
        sequential().hashMap(default)
...
private fun <T> prop(
    ...
    if (value == null) {
        val size = src.size
        val kind = if (rawValue == null && index >= size) "missing" else "invalid"
