...
import io.github.retribution.xposed.tweaks.plugins.DISCORD_OTA_COMMIT
...
val caches by tweak {
    ...
    withAppContext { ctx ->
        val RetributionCacheDir = File(ctx.cacheDir, "Retribution").apply { ensureDir() }
        ...
        val modulesCacheFile by lazy { File(RetributionCacheDir, cacheFileName("modules")).apply { ensureFile() } }
        val assetsCacheFile by lazy { File(RetributionCacheDir, cacheFileName("assets")).apply { ensureFile() } }

        var modulesCache: ModulesCache? = null
        var assetsCache: AssetsCache? = null

        with(RetributionBridgeRegistry) {
            registerMethod("Retribution.caches.modules.read") {
                (modulesCache ?: ModulesCache.loadFromFileOrNull(modulesCacheFile, log)?.also { modulesCache = it })
                    ?.toMap()
            }

            registerMethod("Retribution.caches.modules.write") { args ->
            ...
            registerMethod("Retribution.caches.assets.read") {
                (assetsCache ?: AssetsCache.loadFromFileOrNull(assetsCacheFile, log)?.also { assetsCache = it })
                    ?.toMap()
            }

            registerMethod("Retribution.caches.assets.write") { args ->
                val argv = args.asDelegate()
