...
import io.github.retribution.xposed.tweaks.bridge.RetributionBridgeRegistry.dispatchScope
import io.github.retribution.xposed.tweaks.bridge.RetributionBridgeRegistry.jsCallableLock
import io.github.retribution.xposed.tweaks.bridge.RetributionBridgeRegistry.toNativeObject
...
object RetributionBridgeRegistry : RetributionBridge {
    internal val log: Logger = logger("RetributionBridge")
    ...
    internal const val JS_CALLABLE_MODULE_NAME = "RetributionBridge"
    internal const val NATIVE_CALL_DATA_KEY = "Retribution"
    ...
    internal fun setReflectiveMethods(
        ...
        RetributionBridgeRegistry.argumentsInstance = argumentsInstance
        argumentsMakeNativeObject = makeNativeObject
        readableMapToHashMap = toHashMap
    }

    /**
     * Returns the wrapped `{result | error}` payload if [rawCallData] was for Retribution bridge,
     ...
     * Used by the built-in `Retribution.__callableReturn` bridge method registered in [RetributionBridgeSupport].
     */
...
val RetributionBridgeSupport by tweak {
    RetributionBridgeRegistry.clearMethods()
    ...
    RetributionBridgeRegistry.setReflectiveMethods(
        callFunctionOnModule,
        ...
        classLoader.loadClass("com.horcrux.svg.RNSVGRenderableManager")
            ...
            .hook {
                before {
                    ...
                    val response = RetributionBridgeRegistry.tryDispatchNative(callData) ?: return@before
    ...
    classLoader.loadClass("com.facebook.react.modules.blob.FileReaderModule")
        ...
        .hook {
            before {
                ...
                val handled = RetributionBridgeRegistry.tryDispatchNativeAsync(callData) { response ->
    ...
    findAndHookConstructor(
        ...
        methodHook {
            after {
                log.i("Captured ReactInstance for RetributionBridge")
                RetributionBridgeRegistry.captureReactInstance(thisObject!!)
            }
        }.build(),
    )

    // Native -> JS return (async, `Retribution.__callableReturn`)
    RetributionBridgeRegistry.registerMethod("Retribution.__callableReturn") { args ->
        @Suppress("UNCHECKED_CAST")
        val data = args.firstOrNull() as? Map<String, Any?> ?: emptyMap()
        RetributionBridgeRegistry.completeNextJsCallable(
            result = data["result"],
            error = data["error"],
        )
        null
    ...
    RetributionBridgeRegistry.registerMethod("Retribution.info") {
        mapOf(
            "name" to RetributionConstants.LOADER_NAME,
            "version" to RetributionConstants.LOADER_VERSION,
        )
    }
}
