...
package io.github.retribution.xposed.tweaks.legacy.appearance
...
import io.github.retribution.xposed.tweaks.legacy.RetributionPayloadBuilder
...
val fonts by tweak {
    ...
    RetributionPayloadBuilder.contribute { put("fontPatch", 2) }
    ...
    withAppContext { ctx ->
        ...
        val fontDefFile = File(dataDir, "${RetributionConstants.FILES_DIR}/fonts.json").apply { ensureFile() }
        if (!fontDefFile.exists()) return@withAppContext

        val fontDef = try {
            RetributionJson.decodeFromString<FontDefinition>(fontDefFile.readText())
        } catch (e: Throwable) {
        ...
        val downloadsDir = File(dataDir, "${RetributionConstants.FILES_DIR}/downloads/fonts").apply { ensureDir() }
