...
val themes by tweak {
    ...
    val themeFile = File(
        appInfo.dataDir,
        "${RetributionConstants.FILES_DIR}/current-theme.json",
    ).apply { ensureFile() }

    val loaded: Theme? = if (themeFile.exists() && themeFile.readText().let {
            it.isNotBlank() && it != "{}" && it != "null"
        }) {
        runCatching { RetributionJson.decodeFromString<Theme>(themeFile.readText()) }
            .onFailure { log.w("current-theme.json malformed: ${it.message}") }
            .getOrNull()
    } else null

    RetributionPayloadBuilder.contribute {
        put("hasThemeSupport", true)
        if (loaded != null) put("storedTheme", RetributionJson.encodeToJsonElement<Theme>(loaded))
