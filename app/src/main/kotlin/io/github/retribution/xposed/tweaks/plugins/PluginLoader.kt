...
import io.github.retribution.xposed.tweaks.plugins.repos.*
...
val pluginLoader by tweak {
    ...
    registerNativeMethod("Retribution.plugins.getConstants") {
        mapOf(
            "storageRootPath" to pluginStorageRoot(appInfo.dataDir).absolutePath,
        )
    }

    registerNativeMethod("Retribution.plugins.list") {
    ...
    registerNativeMethod("Retribution.plugins.installFile") {
        promptInstallPlugin(
            this,
            scope,
    ...
    registerNativeMethod("Retribution.plugins.startNative") { args ->
    ...
    registerNativeMethod("Retribution.plugins.uninstall") { args ->
        val argv = args.asDelegate()
        val pluginId by argv.string()
    ...
    registerNativeMethod("Retribution.plugins.setEnabled") { args ->
        val argv = args.asDelegate()
        val pluginId by argv.string()
        ...
        scope.launch {
            runCatching {
                tweakCtx.callJSMethod(
                    "Retribution.plugins.states.update",
