...
/**
 * Plugin-state persistence + exposes `Retribution.plugins.states.*` bridge methods.
 */
val pluginStates by tweak {
    ...
    with(RetributionBridgeRegistry) {
        registerMethod("Retribution.plugins.states.read") {
            PluginStatesStore.ensureLoaded(dataDir).toMap()
        }

        registerMethod("Retribution.plugins.states.requestNextBootDefaultsOnly") {
...
/**
 * Persisted to `files/Retribution/plugins/states`.
 */
object PluginStatesStore {
    private const val DATA_DIR = "files/Retribution/plugins"
