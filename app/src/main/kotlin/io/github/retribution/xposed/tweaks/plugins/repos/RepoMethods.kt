package io.github.retribution.xposed.tweaks.plugins.repos

import io.github.retribution.plugins.PluginManifest
import io.github.retribution.xposed.api.registerNativeAsyncMethod
import io.github.retribution.xposed.tweak
import io.github.retribution.xposed.tweaks.plugins.EVENT_REPO_STATE_UPDATE
import io.github.retribution.xposed.tweaks.plugins.emitPluginEvent
import io.github.retribution.xposed.tweaks.plugins.internal.internalPlugins
...
/**
 * Repository management: exposes `Retribution.plugins.repos.*` bridge methods.
 *
...
val pluginRepos by tweak {
    ...
    /**
     * `Retribution.plugins.repos.list() -> Repo[]`
     *
     ...
     */
    registerNativeAsyncMethod("Retribution.plugins.repos.list") {
        buildList {
    ...
    /**
     * `Retribution.plugins.repos.set(config: Array<{ url, enabled? }>) -> null`
     *
     * Replaces the whole user repository list. Array order is priority order.
     */
    registerNativeAsyncMethod("Retribution.plugins.repos.set") { args ->
    ...
    /**
     * `Retribution.plugins.repos.refresh(url) -> Repo`
     *
     * Refreshes one repository's cached index.
     * Failures leave cache unmodified and throws as a bridge error.
     */
    registerNativeAsyncMethod("Retribution.plugins.repos.refresh") { args ->
    ...
    /**
     * `Retribution.plugins.repos.listPlugins(url) -> RepoPluginListing[]`
     *
     * Lists one repository's plugins from its cached index.
     * Internal plugins are served with no artifacts.
     */
    registerNativeAsyncMethod("Retribution.plugins.repos.listPlugins") { args ->
        val url = args.firstOrNull() as? String ?: throw Error("Expected a repository URL")
...
internal fun internalRepoJSPayload(): Map<String, Any?> = mapOf(
    ...
    "internal" to true,
    "name" to "Retribution",
    "description" to "Plugins built into Retribution.",
    "icon" to null,
)
