 ...
 */
internal const val INTERNAL_REPO_URL = "Retribution://internal"
 ...
 * Directory layout:
 * ```
 * files/Retribution/plugins/repos/
...
internal object RepoStore {
    ...
    private fun loadFromFileOrNull(file: File): List<UserRepo>? {
        ...
        return try {
            val config = RetributionJson.decodeFromString<ReposConfig>(file.readText())
    ...
    private fun save(file: File, entries: List<UserRepo>) {
        ...
        try {
            ...
            fos.write(RetributionJson.encodeToString(ReposConfig(repos = entries)).toByteArray())
