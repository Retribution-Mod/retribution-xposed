...
/**
 * Persistence for plugin provenance keyed by plugin ID, `files/Retribution/plugins/repos/sources.json`.
 */
internal object SourcesStore {
    private const val REPOS_DIR = "files/Retribution/plugins/repos"
    ...
    private fun persist(updated: Map<String, PluginSource>) {
        ...
        try {
            ...
            fos.write(RetributionJson.encodeToString(SourcesConfig(sources = updated)).toByteArray())
    ...
    private fun loadFromFileOrNull(f: File): Map<String, PluginSource>? {
        ...
        return try {
            val config = RetributionJson.decodeFromString<SourcesConfig>(f.readText())
