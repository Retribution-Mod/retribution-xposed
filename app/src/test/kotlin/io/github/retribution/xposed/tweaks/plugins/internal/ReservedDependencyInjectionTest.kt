...
/**
 * Every internal plugin gains `Retribution.api` and `discord` at the ANY range. The providers themselves are exempt.
 */
class ReservedDependencyInjectionTest {
    ...
    fun `providers carry the reserved ids and essential internal flags`() {
        ...
        for (provider in listOf(apiProviderPlugin, discordProviderPlugin)) {
            assertTrue(io.github.retribution.xposed.tweaks.plugins.InternalPluginFlags.INTERNAL in provider.internalFlags)
            assertTrue(io.github.retribution.xposed.tweaks.plugins.InternalPluginFlags.ESSENTIAL in provider.internalFlags)
        }
    }
}
